...
public final class OrthogonalLinkRenderer implements LinkRenderer {
    ...
    public void render(UICommandBuilder builder, String selector,
        ...
        if (from.y() != to.y()) {
            ...
            rectangle(builder, selector, to.x() - THICKNESS / 2, top, THICKNESS, height, state);
        }
        if (from.x() != to.x()) {
            int left = Math.min(from.x(), to.x());
            int width = Math.abs(to.x() - from.x()) + THICKNESS / 2;
            rectangle(builder, selector, left, from.y() - THICKNESS / 2, width, THICKNESS, state);
        }
    }

    private static void rectangle(UICommandBuilder builder, String selector,
